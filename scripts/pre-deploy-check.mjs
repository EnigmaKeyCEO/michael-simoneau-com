...
const socialMediaImages = [
  { name: 'Open Graph image', paths: ['public/og-image.jpg', 'public/og-image.png', 'public/og-image.svg'] },
  { name: 'Favicon', paths: ['public/favicon.ico', 'public/favicon.png', 'public/favicon.svg'] },
];
...
for (const asset of socialMediaImages) {
  ...
  if (resolvedPath) {
    ...
    if (fileSizeInBytes > 100) {
      ...
      console.log(
        chalk.red(`✗ ${asset.name} exists but may be empty or corrupt (${candidate}, ${fileSizeInBytes} bytes)`),
      );
...
if (existsSync(packageJsonPath)) {
  try {
    ...
    if (missingScripts.length === 0) {
      console.log(chalk.green('✓ Required npm scripts exist (build, release)'));
    } else {
...
if (existsSync(netlifyConfigPath)) {
  try {
    ...
    if (hasBuildSection && hasCommand && hasPublish) {
      ...
      console.log(chalk.red('✗ Netlify config is missing build.command or build.publish definitions'));
